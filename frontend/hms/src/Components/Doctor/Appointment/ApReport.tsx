...
const ApReport = ({ appointment }: any) => {
  ...
  const form = useForm({
    ...
    validate: {
      symptoms: (value) =>
        value.length > 0 ? null : "please select atleast one symptom",
      ...
      prescription: {
        medicines: {
          ...
          duration: (value) =>
            value > 0 ? null : "Duration must be greater than zero",
  ...
  const handleSubmit = (values: typeof form.values) => {
    let data = {
      ...
      prescription: {
        medicines: values.prescription.medicines.map((med) => ({
          ...med,
          medicineId:
            med.medicineId && med.medicineId !== "OTHER"
              ? med.medicineId
              : null,
        })),
        doctorId: appointment.doctorId,
    ...
    createAppointmentReport(data)
      ...
      .catch((error) => {
        errorNotification(
          error?.response?.data?.errorMessage || "Failed to create report",
        );
      })
      .finally(() => {
        setLoading(false);
              ...
              (med: Medicine, index: number) => (
                <Fieldset className="grid col-span-2 gap-4 grid-cols-2">
                  ...
                  <Select
                    ...
                    onChange={(event: any) => handleChangeMed(index, event)}
                    data={[
                      ...medicine
                        .filter(
                          (x) =>
                  ...
                  {/* <Select
                    ...
                    withAsterisk
                    data={["oral", "Intravenous", "Topical", "Inhalataion"]}
                  /> */}
                  <Select
                    disabled={med.medicineId !== "OTHER"}
                    {...form.getInputProps(
